...
**Casing conventions:** The backend normalizes `sector`, `section`, `format`, and `resourceType` case-insensitively when validating requests. Service IDs in DID Documents should be published in canonical lowercase for these components (e.g., `#identity:openid:device:_dcr`). The `action` component remains case-sensitive (e.g., `_batch`, `_dcr`, `token`).
...
```json
{
  ...
  "body": {
    ...
    "jwks_uri": "did:web:api.acme.org:employee:<email-address>:device:<device-uuid>",
    "jwks": {
      "keys": [
        {
          "alg": "ML-DSA-44",
...
-   **`scope`:** Defines the permissions requested, following SMART v2.0 syntax (e.g., `organization/PractitionerRole.crus`).  
    **Extension:** the scope string MAY include query parameters to bind the request to a specific subject and IPS sections, e.g.  
    `patient/Composition.rs?subject=did:web:api.acme.org:individual:multibase:z...&section=LOINC|48765-2`.  
    This is a extension (not SMART v2 standard) and is validated against consent rules.

**TODO (Policy Behavior):** At present, the backend rejects scopes that exceed the consented permissions. A future option is to emit a reduced scope (subset) and return the granted scope so clients can detect missing permissions and notify users.
...
***Example (GDC extension) requesting a subject- and section-scoped token:***
In this model:
- `iss` is the **device profile DID** (client_id) used to sign the request.
- `sub` is the **user/actor DID** requesting access (employee or family member with a role).
- The target **patient/individual** is bound inside the `scope` parameters.
```bash
curl -X POST 'http://localhost:3000/acme/cds-es/v1/health-care/identity/openid/auth/token' \
--header 'Content-Type: application/didcomm-plaintext+json' \
--data '{
  ...
  "body": {
    ...
    "scope": "patient/Composition.rs?subject=did:web:api.acme.org:individual:multibase:z<unified-health-id>&section=LOINC|48765-2"
  }
}'
```
    ...
    "entry": [{
      meta: {
        claims: {
          ...
          "Communication.sent": "2026-10-15T14:30:00Z",        
    ...
    "entry": [{
      "meta": {
        "claims": {
          ...
          "Composition.date": "2026-11-26T10:00:00Z",
          ...
          "Composition.title": "Allergies and Intolerances: hospital.example.com (2026-11-26T10:00:00Z)",
      ...
      "resource": {
        ...
        "subject": { "reference": "did:web:api.acme.org:individual:<unified-health-identifier>" },
        "status": "final",
        "title": "Allergies and Intolerances: hospital.example.com (2026-11-26T10:00:00Z)",
        "type": {
          "coding": [{
...
--data '{
  ...
  "body": {
    ...
    "resourceType": "Bundle",
    "timestamp": "2026-02-10T14:30:00+01:00",
    "type": "document",
    "entry": [
      {
        ...
        "resource": {
          ...
          "date": "2026-02-10T14:30:00+01:00",
          "author": [
            {
          ...
          "section": [
            {
              ...
              "text": {
                "status": "generated",
                "div": "<div xmlns=\"http://www.w3.org/1999/xhtml\"><p><strong>Maria Rossi (RSSMRA97L67Z602C)</strong></p><p><span>COVID-19 mRNA Vaccine&nbsp;</span></p><p>Last Update: 2026-02-05</p><ol><li>Dose 1/2<ol><li>Date and time of vaccination: 2026-01-10</li><li><span>AIC#049283: COVID-19 Vaccine Moderna (MODERNA BIOTECH SPAIN, S.L.) - Lot#B1235742&nbsp;</span></li><li>Ospedale Careggi - Firenze</li></ol></li><li>Dose 2/2<ol><li>Date vaccine due: 2026-02-07</li><li>Status: contraindicated (Medical Precaution)</li></ol></li></ol></div>"
              },
              "entry": [
                {
