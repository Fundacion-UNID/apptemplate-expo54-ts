// data/hl7-personal-relationship.ts
// Source: HL7 PersonalRelationshipRoleType (subset of display/definition)

export type Hl7PersonalRelationshipRole = {
  code: string;
  display: string;
  definition: string;
};

const ROLE_LIST: Hl7PersonalRelationshipRole[] = [
...
const roleByCode = new Map(ROLE_LIST.map((role) => [role.code, role]));
const preferred = PREFERRED_ORDER.map((code) => roleByCode.get(code)).filter(Boolean) as Hl7PersonalRelationshipRole[];
const remaining = ROLE_LIST.filter((role) => !PREFERRED_ORDER.includes(role.code));

export const HL7_PERSONAL_RELATIONSHIP_ROLES: Hl7PersonalRelationshipRole[] = [
  ...preferred,
  ...remaining,
];
