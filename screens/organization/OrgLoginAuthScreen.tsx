// screens/organization/OrgLoginAuthScreen.tsx
// REASON: This screen is the definitive entry point for the login flow.
// It solely handles authentication and then passes the verified user credentials to the next step.
...
export default function OrgLoginAuthScreen() {
  const { t } = useTranslation();
  const navigation = useNavigation<any>();
  const route = useRoute<RouteProp<{ OrgLoginAuth?: { nextRoute?: string; nextParams?: Record<string, unknown> } }, 'OrgLoginAuth'>>();
  const screenStyles = getScreenStyles();
  const backgroundColor = useThemeColor({}, 'background');
  const auth = useAuth();
  const { setSubject } = useSubject();
  const { profileRegistry } = useProfile();
  const { nextRoute, nextParams } = route.params || {};
  ...
  // Effect for post-authentication navigation
  useEffect(() => {
        ...
        if (profileRegistry && !nextRoute) {
          ...
          const profiles = await profileRegistry.list({
            where: [
              ...
              { attribute: 'appType', equals: 'organization' },
            ],
          });
          if (profiles.length > 0) {
            navigation.navigate(Routes.Organization.ProfileSelect.name, { email, idToken });
            return;
          }
        }

        const targetRoute = nextRoute || Routes.Organization.ProfileSelect.name;
  ...
  const handleAppleSignIn = async () => {
    try {
      ...
      if (appleCredential.identityToken) {
        await auth.handleSignIn(() => signInWithApple(appleCredential.identityToken as string));
      }
    } catch (e: any) {
