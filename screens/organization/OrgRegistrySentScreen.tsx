// screens/organization/OrgRegistrySentScreen.tsx
...
export default function OrgRegistrySentScreen({ route, navigation }: Props) {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      if (!found) return;
      if (found.status !== JobStatus.COMPLETED) return;
  ...
  const handleConfirmOrder = useCallback(async () => {
    ...
    const orgAdmin = profileManager.orgAdmin?.admin;
    if (!orgAdmin) {
      setError('Admin services are not available for this session (missing OrgAdminService).');
      return;
    }
    ...
    try {
      ...
      navigation.replace(Routes.Organization.RegistrySent.name, { thid: orderThid });
    } catch (e) {
      setError((e as Error).message);
    } finally {
      setIsBusy(false);
  ...
  return (
    <View style={{ flex: 1, backgroundColor, justifyContent: 'center', padding: 20 }}>
      <ScreenHeader
        title={t('organization.screens.registrySent.title', 'Organization onboarding')}
        subtitle={t('organization.screens.registrySent.subtitle', 'Submitting and waiting for async processing')}
        description={t(
          'organization.screens.registrySent.description',
      ...
      <ThemedText style={{ textAlign: 'center', opacity: 0.8, marginBottom: 8 }}>
        {t('organization.screens.registrySent.status', 'Status')}: {statusLabel}
      </ThemedText>
      ...
      {!job || job.status !== JobStatus.COMPLETED ? (
        ...
        <>
          {offerId && (
            <>
              <ThemedText style={{ textAlign: 'center', opacity: 0.8, marginTop: 16 }}>
                {t('organization.screens.registrySent.offerId', 'Offer')}:
              </ThemedText>
              <ThemedText selectable style={{ fontFamily: 'monospace', marginVertical: 8, textAlign: 'center' }}>
                {offerId}
              </ThemedText>
          ...
          {activationCode && (
            <>
              <ThemedText style={{ textAlign: 'center', opacity: 0.8, marginTop: 16 }}>
                {t('organization.screens.registrySent.activationCode', 'Activation code')}:
              </ThemedText>
              <ThemedText selectable style={{ fontFamily: 'monospace', marginVertical: 8, textAlign: 'center' }}>
                {activationCode}
              </ThemedText>
              <ThemedButton
                ...
                onPress={() => navigation.navigate(Routes.Organization.Login.name)}
                disabled={isBusy}
              />
            </>
          )}
